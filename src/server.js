...
const nameByPid = new Map();  // для восстановления игрока в комнате сразу после «reset» (Переиграть)
const pfpByPid = new Map();
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (buf) => {
    ...
    if (m.type === 'reset') {
      // Переиграть: создаём свежую комнату с теми же настройками (isPublic/withBots/title)
      // и возвращаем в неё всех, кто был в этой комнате (чтобы всем за столом сразу была
      // видна новая партия без ручного перезахода).
      const old = G.getRoom(c.roomId);
      const fresh = G.newRoom(c.roomId, { isPublic: old.isPublic, withBots: old.withBots, title: old.title });
      G.rooms[c.roomId] = fresh;
      for (const [otherWs, oc] of clients) {
        if (oc.roomId !== c.roomId || !oc.pid) continue;
        const name = nameByPid.get(oc.pid) || 'Player';
        G.addPlayer(fresh, oc.pid, name, false, pfpByPid.get(oc.pid) || null);
      }
      if (fresh.withBots) {
        const humans = fresh.order.filter(id => !fresh.players[id].isBot).length;
        const bots = fresh.order.filter(id => !!fresh.players[id].isBot).length;
        let need = Math.max(0, 4 - humans - bots);
        for (let i = 0; i < need; i++) {
          const bn = G.BOT_NAMES[(bots + i) % G.BOT_NAMES.length];
          G.addPlayer(fresh, 'bot_' + bn.replace(/\s/g, '') + '_' + i, bn, true);
        }
      }
      broadcast(c.roomId);
      return;
