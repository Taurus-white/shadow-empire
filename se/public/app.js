...
const DICT = {
  ru: {
    ...
    endRound: 'Завершить раунд', endRoundWait: 'Ждём остальных…', gameOver: 'Игра окончена',
    ...
    chainsTitle: 'ЦЕПОЧКИ ЭТОЙ ПАРТИИ', chainsHint: 'В каждой цепочке 4 предприятия. Видно только 2 из 4 — остальные два угадай по загадкам ниже. Собери все 4 — множитель аренды и бонус, первому — вдвое.',
    chainRevealHint: n => `Раскрыто ${n} из 4`, chainYours: 'у тебя',
  ...
  en: {
    ...
    endRound: 'End round', endRoundWait: 'Waiting for others…', gameOver: 'Game over',
    ...
    chainsTitle: 'CHAINS IN THIS GAME', chainsHint: 'Each chain has 4 businesses. Only 2 of 4 are revealed — guess the rest from the riddles below. Collect all 4 for a rent multiplier and bonus, double for first.',
    chainRevealHint: n => `${n} of 4 revealed`, chainYours: 'yours',
...
function render() {
  ...
  if (top && top.t !== lastLogTop) {
    lastLogTop = top.t;
    // вытянутая карта / покупка — анимация в центр поля на 1.7 сек.
    // ВАЖНО: вставляем оверлей ПОСЛЕ отрисовки и НЕ перерисовываем пока он активен,
    // иначе renderBoard() затрёт centerCardSlot.
    if (top.key === 'log_card' || top.key === 'log_buy' || top.key === 'log_buyout' || top.key === 'log_accept_offer') {
      overlayActive = true;
      renderBoard();
      if (top.key === 'log_card') showCardOverlay(top);
      else showBuyOverlay(top);
      setTimeout(() => { overlayActive = false; }, 1700);
    }
    if (['world', 'law', 'attack', 'buy', 'corrupt', 'win'].includes(top.kind)) {
      const mineEvent = top.actorId === PID || top.targetId === PID;
      const text = logText(top);
...
let lastCardT = 0;
function showCardOverlay(e) {
  if (e.t === lastCardT) return;
  lastCardT = e.t;
  ...
  const icon = isChance ? '🎯' : '🎁';
  // карточка встаёт в центр поля (centerCardSlot), с flip-анимацией
  const slot = $('centerCardSlot');
  if (slot) {
    slot.innerHTML = `<div class="card-center ${isChance ? '' : 'chest'} flip-in">
    ...
    clearTimeout(slot._h);
    slot._h = setTimeout(() => { slot.innerHTML = ''; }, 1700);
  }
}

// анимация покупки недвижимости — слева достопримечательность города, справа объект (8-бит)
let lastBuyT = 0;
function showBuyOverlay(e) {
  if (e.t === lastBuyT) return;
  lastBuyT = e.t;
  const allProps = S.zones.flatMap(z => z.assets);
  ...
  const slot = $('centerCardSlot');
  if (slot) {
    slot.innerHTML = `<div class="buy-center buy-in">
      <div class="bc-side">
...
let diceAnim = 0, overlayActive = false;
...
let animTokenPid = null, animTo = null, animStep = 0, animTimer = null;
// Анимация движения фишки по клеткам: временно подменяем позицию игрока
function tickTokenAnim() {
  if (animTokenPid == null) return;
  const me = S.players.find(p => p.id === animTokenPid);
  if (!me) { animTokenPid = null; return; }
  let cur = me.pos || 0;
  cur = (cur + 1) % 40;
  me.pos = cur;
  animStep++;
  renderBoardOnly();
  if (cur === animTo || animStep > 40) {
    animTokenPid = null;
    me.pos = animTo;
    renderBoardOnly();
  } else {
...
function renderBoard() {
  if (!S.board) return;
  // если активен оверлей карты/покупки — не перерисовываем, иначе затрём анимацию
  ...
  const cells = S.board.map(c => {
    ...
    const toks = S.players.filter(p => !p.eliminated && (p.pos || 0) === c.i)
      .map(p => {
        const isMe = p.id === PID;
        const inner = p.pfp
    ...
    if (c.type === 'prop') {
      ...
      if (!a) return '';
      const own = c.owner === PID ? 'own-me' : (c.owner ? 'own-rival' : '');
  ...
  if (isNew) {
    diceAnim = Date.now();
    // запускаем анимацию движения фишки от from к to
    if (lr.from != null && lr.from !== lr.to && animTokenPid == null) {
      animTokenPid = lr.pid;
      animTo = lr.to;
      animStep = 0;
      clearTimeout(animTimer);
      const starter = S.players.find(p => p.id === lr.pid);
      if (starter) { starter.pos = lr.from; }
      tickTokenAnim();
    }
    // если это МОЙ бросок и я встал на свободную клетку-бизнес — авто-открыть шит покупки
    if (lr.pid === PID && lr.event?.type === 'prop_free' && !window._lastAutoBuy) {
      window._lastAutoBuy = lr.at;
      const a = allProps.find(x => x.id === lr.event.propId);
      if (a) setTimeout(() => { if (!document.getElementById('sheet').classList.contains('hidden')) return; showAsset(a.zone, a.id); }, 1700);
    }
  }
...
function renderChains() {
  ...
  $('tab-chains').innerHTML = `<div class="mini" style="margin:0 2px 9px">${t('chainsHint')}</div>
    ...
    return `<div class="chain ${done ? 'done' : ''}">
      ...
      <div class="cd">${t('chainRevealHint', revealedCount)}</div>
...
function renderMap() {
  const html = S.zones.map(z => {
    ...
    const assets = z.assets.map(a => {
      ...
      return `<div class="asset ${cls} ${a.frozen ? 'frozen' : ''}" onclick="showAsset('${z.id}','${a.id}')">
...
window.showAsset = (zid, aid) => {
  ...
  if (!a.owner) {
    ...
    const rentBtn = a.rentDue > 0
      ? `<div class="rowbtns"><button class="btn" onclick="act('pay_rent',{assetId:'${a.id}'})">${t('payRentBtn', money(a.rentDue), a.kind === 'crime' ? t('buyBlack') : '')}</button></div>`
      : '';
    btns = `${rentBtn}
      ...
      ${a.damaged && myMayor ? `<div class="rowbtns"><button class="btn danger" onclick="act('seize',{assetId:'${a.id}'})">${t('seizeBtn')}</button></div>` : ''}
