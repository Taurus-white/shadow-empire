...
function applyCard(room, p, card, deps) {
  ...
  switch (card.kind) {
    ...
    case 'pct': {
      const total = Math.round(propertyValue(room, p.id, valueFn) * card.pct);
      const r = chargeOrSell(room, p, total, valueFn, log);
...
function doRoll(room, p, deps) {
  ...
  return { d1, d2, steps, from, to, passedStart, event };
}

module.exports = {
  T, rollDie, doRoll, resolveCell, applyCard, drawCard,
