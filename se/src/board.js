...
/** В каждой партии активны только 3 случайные цепочки, и у каждой раскрыты только 2 из 4 слотов. */
function pickChains(rnd = Math.random) {
  const pool = [...CHAIN_TEMPLATES];
  const out = [];
  while (out.length < 3 && pool.length) {
    const tpl = pool.splice(Math.floor(rnd() * pool.length), 1)[0];
    const idxs = [0, 1, 2, 3].sort(() => rnd() - 0.5);
    const revealIdx = idxs.slice(0, 2).sort((a, b) => a - b);
    out.push({ ...tpl, revealIdx });
  }
  return out;
}
...
const CHANCE = [
  { id: 'ch_go',       kind: 'move',    to: 0,      text: 'На старт. Получи $200',                    textEn: 'Advance to GO. Collect $200',          amount: M(200) },
  { id: 'ch_div',      kind: 'gain',    amount: M(50),   text: 'Банк платит дивиденд $50',            textEn: 'Bank pays you a dividend of $50' },
  { id: 'ch_jail',     kind: 'jail',    text: 'Отправляйся в тюрьму',                                 textEn: 'Go to Jail' },
  { id: 'ch_repair',   kind: 'perprop', amount: M(100),  text: 'Общий ремонт: $100 за каждый объект', textEn: 'General repairs: $100 per property' },
  { id: 'ch_speed',    kind: 'pay',     amount: M(150),  text: 'Штраф за превышение скорости $150',   textEn: 'Speeding fine $150' },
  { id: 'ch_doc',      kind: 'pay',     amount: M(500),  text: 'Счёт от врача. Заплати $500',         textEn: "Doctor's fee. Pay $500" },
  { id: 'ch_refund',   kind: 'gain',    amount: M(2000), text: 'Возврат налога. Получи $2000',        textEn: 'Income tax refund. Collect $2000' },
  { id: 'ch_bday',     kind: 'fromall', amount: M(100),  text: 'День рождения: по $100 с каждого',    textEn: 'Birthday: collect $100 from each player' },
  { id: 'ch_life',     kind: 'gain',    amount: M(1000), text: 'Страховка жизни. Получи $1000',       textEn: 'Life insurance matures. Collect $1000' },
  { id: 'ch_inherit',  kind: 'gain',    amount: M(10000),text: 'Наследство $10 000',                  textEn: 'You inherit $10,000' },
  { id: 'ch_taxerr',   kind: 'pay',     amount: M(1500), text: 'Ошибки в декларации. Заплати $1500',  textEn: 'Tax filing errors. Pay $1,500' },
  { id: 'ch_audit',    kind: 'pay',     amount: M(2000), text: 'Налоговая проверка. Заплати $2000',   textEn: 'Tax audit. Pay $2,000' },
  { id: 'ch_fire',     kind: 'pay',     amount: M(200),  text: 'Пожарная инспекция. Заплати $200',    textEn: 'Fire inspection. Pay $200' },
  { id: 'ch_robbed',   kind: 'pay',     amount: M(300),  text: 'Салон обокрали. Заплати $300',        textEn: 'Your store was robbed. Pay $300' },
  { id: 'ch_pipes',    kind: 'pay',     amount: M(500),  text: 'Прорвало трубы. Заплати $500',        textEn: 'Pipes burst. Pay $500' },
  { id: 'ch_insider',  kind: 'pay',     amount: M(500),  text: 'Вор среди своих. Заплати $500',       textEn: 'A thief among your own. Pay $500' },
];

const CHEST = [
  { id: 'cc_labor',    kind: 'perprop', amount: M(100),  text: 'Трудовая инспекция: $100 за объект',  textEn: 'Labor inspection: $100 per property' },
  { id: 'cc_vat',      kind: 'pct',     pct: 0.10,       text: 'Пеня по НДС: 10% стоимости имущества',textEn: 'VAT penalty: 10% of property value' },
  { id: 'cc_eco',      kind: 'pay',     amount: M(750),  text: 'Экологический штраф $750',            textEn: 'Environmental penalty $750' },
  { id: 'cc_govt',     kind: 'gain',    amount: M(20000),text: 'Госконтракт выполнен. Получи $20 000',textEn: 'Government contract. Collect $20,000' },
  { id: 'cc_patent',   kind: 'pay',     amount: M(1500), text: 'Проигран патентный спор. $1500',      textEn: 'Lost patent lawsuit. Pay $1,500' },
  { id: 'cc_cyber',    kind: 'pay',     amount: M(1000), text: 'Кибератака. Восстановление $1000',    textEn: 'Cyberattack. Pay $1,000 to restore' },
  { id: 'cc_charity',  kind: 'pay',     amount: M(500),  text: 'Благотворительный взнос $500',        textEn: 'Charity contribution $500' },
  { id: 'cc_sponsor',  kind: 'pay',     amount: M(750),  text: 'Спонсорство конференции $750',        textEn: 'Event sponsorship $750' },
  { id: 'cc_innov',    kind: 'gain',    amount: M(10000),text: 'Премия за инновации $10 000',         textEn: 'Innovation award $10,000' },
  // 7 своих карт, 50/50 хороших и плохих
  { id: 'cc_leak',     kind: 'pay',     amount: M(1200), text: 'Утечка в прессу: PR-расходы $1200',   textEn: 'Press leak: PR costs $1,200' },
  { id: 'cc_whistle',  kind: 'pay',     amount: M(900),  text: 'Информатор в компании. $900 «на решение»', textEn: 'Whistleblower inside. Pay $900 to handle it' },
  { id: 'cc_sanction', kind: 'pct',     pct: 0.06,       text: 'Санкционный комплаенс: 6% имущества', textEn: 'Sanctions compliance: 6% of property' },
  { id: 'cc_tender',   kind: 'gain',    amount: M(8000), text: 'Выиграл закрытый тендер. $8000',      textEn: 'Won a closed tender. Collect $8,000' },
  { id: 'cc_amnesty',  kind: 'gain',    amount: M(5000), text: 'Налоговая амнистия. Возврат $5000',   textEn: 'Tax amnesty. Refund $5,000' },
  { id: 'cc_offshore', kind: 'gain',    amount: M(6500), text: 'Офшорный дивиденд $6500',             textEn: 'Offshore dividend $6,500' },
  { id: 'cc_grant',    kind: 'gain',    amount: M(7000), text: 'Грант на модернизацию $7000',         textEn: 'Modernization grant $7,000' },
];
